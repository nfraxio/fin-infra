...
async def test_project_net_worth_different_users():
    ...
    assert (
        projection1.current_net_worth != projection2.current_net_worth
        or projection1.monthly_contribution != projection2.monthly_contribution
    )
...
async def test_project_net_worth_growth_with_contributions():
    ...
    simple_growth = projection.current_net_worth * math.pow(1 + conservative.expected_return, 20)
