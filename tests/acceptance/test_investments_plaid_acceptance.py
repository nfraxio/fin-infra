...
class TestPlaidInvestmentsAcceptance:
    ...
    def test_get_holdings_with_sandbox(self, plaid_access_token):
        ...
        print(f"  Sample holding: {security.name or security.ticker_symbol} - {holding.quantity} @ ${holding.institution_price}")
    ...
    def test_get_holdings_filters_by_account(self, plaid_access_token):
        ...
        try:
            ...
            print(f"✓ Account filtering: {len(filtered_holdings)}/{len(all_holdings)} holdings in account {first_account_id}")
    ...
    def test_get_transactions_with_sandbox(self, plaid_access_token):
        ...
        print(f"  Sample: {transaction.transaction_type.value} - {transaction.security.name or transaction.security.ticker_symbol}")
    ...
    def test_get_allocation(self, plaid_access_token):
        ...
        total_percent = sum(
            alloc.percentage for alloc in allocation.allocation_by_asset_class
        )
        assert 99.0 <= total_percent <= 101.0, f"Allocations should sum to 100%, got {total_percent}%"
        
        print(f"✓ Asset allocation calculated from Plaid sandbox holdings:")
    ...
    def test_cost_basis_and_pnl_calculation(self, plaid_access_token):
        ...
        try:
            ...
            if len(holdings_with_cost_basis) == 0:
                pytest.skip("No holdings have cost basis in sandbox (expected for some account types)")
            ...
            print(f"✓ Cost basis and P/L calculation:")
